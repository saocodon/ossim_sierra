...
int set_argument(int current_argument_value, int new_argument_value, int argument_mask, int argument_offset) {
  int entry_with_argument_cleared = (current_argument_value & ~argument_mask);
  int entry_with_only_argument    = ((new_argument_value << argument_offset) & argument_mask);
...
// ==========================================================================
const int PTE_PRESENT_BIT          = 31; // if this entry is present in the table
const int PTE_ONSWAP_BIT           = 30; // if this page is on RAM or on swap
...
#define EXTRACT_OFFSET(entry) ((entry & set_bits(ADDRESS_OFFSET_LOW_BIT, ADDRESS_OFFSET_HIGH_BIT)) >> ADDRESS_OFFSET_LOW_BIT)
